...
Author(s) of Significant Updates/Modifications to the File:
  ...
  Guangyu Yan, 11/11/18, University of Minnesota
*/
...
FlashPhotoApp::FlashPhotoApp(int width, int height,
    ...
    : GraphicsApp(width, height, "FlashPhoto"),
      ...
      quant_bins_(5) {
  image_editor =
    new ImageEditor(new PixelBuffer(width, height, background_color));
}
...
PixelBuffer *FlashPhotoApp::pixel_buffer() {
  return image_editor->pixel_buffer();
}

void FlashPhotoApp::set_pixel_buffer(PixelBuffer *buffer) {
  image_editor->set_pixel_buffer(buffer);
}
